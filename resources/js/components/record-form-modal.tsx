...
const recommendationTypes = [
    {
        value: 'exercise',
        label: 'Exercise / Physical',
        icon: <Activity className="h-4 w-4" />,
    },
    {
        value: 'reading',
        label: 'Reading / Bibliotherapy',
...
export function RecordFormModal({
    ...
    const { data, setData, patch, processing, reset } = useForm({
        ...
        structured_recommendations:
            initialData?.structured_recommendations ?? [],
    });
    ...
    const toggleState = (state: string) => {
        if (data.patient_state.includes(state)) {
            setData(
                'patient_state',
                data.patient_state.filter((s) => s !== state),
            );
        } else {
    ...
    const addCustomState = () => {
        if (
            customState.trim() &&
            !data.patient_state.includes(customState.trim())
        ) {
            setData('patient_state', [
                ...data.patient_state,
                customState.trim(),
            ]);
            setCustomState('');
        ...
        <div className="fixed inset-0 z-[100] overflow-y-auto bg-[#191c1e]/60 backdrop-blur-sm">
            <div className="flex min-h-full items-center justify-center p-4">
                <div className="w-full max-w-3xl rounded-3xl bg-white p-6 shadow-xl sm:p-8">
                    <div className="sticky top-0 z-10 mb-6 flex items-start justify-between gap-4 border-b border-[#f2f4f6] bg-white pb-4">
                        <div>
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="mb-2 flex flex-wrap gap-2">
                                {predefinedStates.map((state) => {
                                    const isActive =
                                        data.patient_state.includes(state);
                                    return (
                                        <button
                                            key={state}
                                            type="button"
                                            ...
                                            className={`rounded-full border px-3 py-1.5 text-xs font-bold transition-colors ${
                                                isActive
                                                    ? 'border-[#1464BC] bg-[#1464BC] text-white'
                                                    : 'border-[#e2e4e6] bg-[#f7f9fb] text-[#717783] hover:bg-[#f2f4f6]'
                                            }`}
                                        >
                                            {state}
                                        </button>
                                ...
                                {data.patient_state
                                    ...
                                    .map((state) => (
                                        <button
                                            ...
                                            type="button"
                                            onClick={() => toggleState(state)}
                                            className="rounded-full border border-[#1464BC] bg-[#1464BC] px-3 py-1.5 text-xs font-bold text-white transition-colors"
                                        >
                                            {state} (Custom)
                                        </button>
                                    ))}
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setData('diagnostic_focus', e.target.value)
                                }
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <textarea
                                ...
                                onChange={(e) =>
                                    setData('record_summary', e.target.value)
                                }
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="flex flex-col gap-3">
                                {data.structured_recommendations.length ===
                                0 ? (
                                    <div className="rounded-xl border border-dashed border-[#e2e4e6] bg-[#f7f9fb] py-4 text-center text-sm text-[#a0a5b1]">
                                        Belum ada rekomendasi.
                                    </div>
                                ) : (
                                    data.structured_recommendations.map(
                                        (rec, idx) => (
                                            <div
                                                key={idx}
                                                className="relative flex flex-col gap-3 rounded-xl border border-[#e2e4e6] bg-[#f7f9fb] p-4 pt-12"
                                            >
                                                <span className="absolute top-4 left-4 text-[11px] font-black tracking-wider text-[#1464BC] uppercase">
                                                    Rekomendasi {idx + 1}
                                                </span>
                                                <button
                                                    type="button"
                                                ...
                                                <div className="flex gap-3">
                                                    <div className="flex-1">
                                                        <input
                                                            ...
                                                            className="h-9 w-full rounded-lg border border-[#e2e4e6] bg-white px-3 text-sm text-[#191c1e] outline-none focus:border-[#1464BC]"
                                                            required
                                                        />
                                                    </div>
                                                    <div className="w-1/3">
                                                ...
                                                <textarea
                                                    ...
                                                    className="h-20 w-full resize-none rounded-lg border border-[#e2e4e6] bg-white p-3 text-sm text-[#191c1e] outline-none focus:border-[#1464BC]"
                                                    required
                                                />
                                            </div>
                                        ),
